...
VOL_PADRAO: dict[str, float] = {
    "acao_br": 0.018, "acao_us": 0.015, "crypto": 0.045,
    "fii": 0.012, "renda_fixa": 0.002, "tesouro": 0.004, "commodities": 0.020,
}
...
def _pct(valor, base):
    return ((valor - base) / base * 100) if base else 0.0

def simular_carteira(carteira, choque, n_simulacoes=N_SIMULACOES, horizonte_dias=HORIZONTE_DIAS):
    ...
    for ativo in carteira.ativos:
        ...
        retorno_choque = aplicar_choque(ativo.classe, choque, ticker=ativo.ticker)
        drift_diario = retorno_choque / horizonte_dias

        ruidos = np.random.normal(loc=drift_diario, scale=vol, size=(n_simulacoes, horizonte_dias))
    ...
    return ResultadoSimulacao(
        ...
        valor_p10=p10, valor_p50=p50, valor_p90=p90,
