...
h1 {{ color: {TEXT_PRI} !important; font-family: 'Syne', sans-serif !important; font-weight: 800; letter-spacing: -0.03em; font-size: 2rem !important; }}
...
[data-testid="stWidgetLabel"] p, [data-testid="stWidgetLabel"] label {{ color: {MUTED} !important; font-size: 0.72rem !important; letter-spacing: 0.06em; text-transform: uppercase; }}
...
[data-testid="metric-container"]:hover {{ border-color: {MUTED}; transform: translateY(-1px); }}
[data-testid="stMetricLabel"] {{ color: {MUTED} !important; font-size: 0.68rem !important; letter-spacing: 0.1em; text-transform: uppercase; font-family: 'DM Mono', monospace !important; }}
[data-testid="stMetricValue"] {{ color: {TEXT_PRI} !important; font-family: 'DM Mono', monospace !important; font-size: 1.5rem !important; font-weight: 500 !important; }}
...
.stButton > button[kind="primary"]:hover {{ background: #2563eb !important; transform: translateY(-1px); }}
...
[data-testid="stProgress"] > div > div {{ background: {ACCENT} !important; }}
...
.var-card h3 {{ color: #f87171 !important; font-family: 'DM Mono', monospace !important; }}

.badge-positivo {{ display: inline-flex; align-items: center; gap: 4px; background: #052e16; color: #4ade80; border: 1px solid #166534; border-radius: 20px; padding: 3px 12px; font-family: 'DM Mono', monospace; font-size: 0.82rem; font-weight: 600; }}
.badge-negativo {{ display: inline-flex; align-items: center; gap: 4px; background: #450a0a; color: #f87171; border: 1px solid #7f1d1d; border-radius: 20px; padding: 3px 12px; font-family: 'DM Mono', monospace; font-size: 0.82rem; font-weight: 600; }}
.badge-neutro {{ display: inline-flex; align-items: center; gap: 4px; background: {SURFACE}; color: {TEXT_SEC}; border: 1px solid {BORDER}; border-radius: 20px; padding: 3px 12px; font-family: 'DM Mono', monospace; font-size: 0.82rem; font-weight: 600; }}

.page-header {{ margin-bottom: 1.5rem; }}
.page-header h1 {{ margin: 0 0 0.2rem 0; }}
.page-header p {{ color: {MUTED}; font-size: 0.82rem; font-family: 'DM Mono', monospace; margin: 0; letter-spacing: 0.04em; }}
...
def page_header(titulo, subtitulo=""):
    ...
    st.markdown(f'<div class="page-header"><h1>{titulo}</h1>{sub}</div>', unsafe_allow_html=True)
...
<div style="display:flex;flex-direction:column;gap:6px;padding:0.25rem 0">
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;">
        <span style="color:#475569;font-size:0.7rem;text-transform:uppercase;letter-spacing:0.08em;font-family:'DM Mono',monospace">P&L</span>
...
if pagina == "🏠  Carteira":
    ...
    with st.expander("➕  Adicionar ativo", expanded=not carteira.ativos):
        tipo = st.radio("Tipo de ativo", ["Ação / FII / Cripto / Commodity", "Tesouro Direto"], horizontal=True)
        c1, c2 = st.columns(2)
        with c1:
    ...
    if not carteira.ativos:
        st.markdown(f"""
<div style="background:{SURFACE};border:1px solid {BORDER};border-radius:12px;padding:2rem;text-align:center;margin-top:1rem;">
    <div style="font-size:2rem;margin-bottom:0.5rem;">📭</div>
    <div style="color:{TEXT_PRI};font-weight:600;margin-bottom:0.25rem;">Carteira vazia</div>
...
""", unsafe_allow_html=True)
    ...
    m3.metric("P&L",            f"R$ {carteira.pl_total_reais:+,.2f}",
    ...
    st.markdown(f'<div style="color:{MUTED};font-size:0.72rem;text-transform:uppercase;letter-spacing:0.08em;font-family:DM Mono,monospace;margin-bottom:0.75rem">Evolução patrimonial — 12 meses</div>', unsafe_allow_html=True)
    with st.spinner(""):
    ...
    if acum is not None and len(acum) > 1:
        ...
        fig_evol.update_layout(yaxis_tickprefix="R$ ", showlegend=False, height=200,
                               margin=dict(t=10, b=40, l=80, r=20), hovermode="x unified")
    ...
    if st.button("🚀  Simular cenário", type="primary", use_container_width=True):
        ...
        if choque_nz:
            ...
            for i, (k, v) in enumerate(choque_nz.items()):
                cols_choque[i].metric(VARIAVEIS_MACRO[k], f"{v:+.1f}%",
        ...
        st.markdown(
            ...
            f"choque direto: {badge_retorno(resultado.impacto_choque_pct)}",
            unsafe_allow_html=True,
        )
        st.markdown("")
        ...
        st.markdown(
            ...
            f'<p style="font-size:0.68rem;letter-spacing:0.1em;text-transform:uppercase;margin:0 0 6px;font-family:DM Mono,monospace">⚠️ Valor em Risco (VaR 90%)</p>'
            f'<h3 style="margin:0">R$ {var_reais:,.2f} <span style="font-size:0.9rem;opacity:0.7">({var_pct:.1f}%)</span></h3>'
            f'<p style="margin:6px 0 0;font-size:0.78rem;opacity:0.8">Perda máxima esperada em 90% dos cenários simulados</p>'
        ...
        with col_hist:
            st.markdown(f'<div style="color:{MUTED};font-size:0.72rem;text-transform:uppercase;letter-spacing:0.08em;font-family:DM Mono,monospace;margin-bottom:0.5rem">Distribuição dos cenários</div>', unsafe_allow_html=True)
            fig = go.Figure()
            fig.add_trace(go.Histogram(
                x=resultado.distribuicao, nbinsx=80,
                marker=dict(color=ACCENT, opacity=0.6, line=dict(color=BG, width=0.3)),
                name="Simulações",
            ))
            ...
            fig.update_layout(xaxis_title="Valor da carteira (R$)", yaxis_title="Frequência",
                              showlegend=False, **plotly_layout())
            st.plotly_chart(fig, use_container_width=True, key="hist_simulador")

        with col_tab:
            st.markdown(f'<div style="color:{MUTED};font-size:0.72rem;text-transform:uppercase;letter-spacing:0.08em;font-family:DM Mono,monospace;margin-bottom:0.5rem">Impacto por ativo</div>', unsafe_allow_html=True)
        ...
        st.divider()
        st.markdown(f'<div style="color:{MUTED};font-size:0.72rem;text-transform:uppercase;letter-spacing:0.08em;font-family:DM Mono,monospace;margin-bottom:0.5rem">🤖 Análise do assessor</div>', unsafe_allow_html=True)
        ...
        st.download_button("📥  Exportar resultado (.txt)", data=resumo_export,
                           file_name=f"simulacao_{cenario_texto[:30].replace(' ','_')}.txt",
                           mime="text/plain")

    if st.session_state.historico_simulacoes:
        st.divider()
        st.markdown(f'<div style="color:{MUTED};font-size:0.72rem;text-transform:uppercase;letter-spacing:0.08em;font-family:DM Mono,monospace;margin-bottom:0.75rem">Histórico de simulações</div>', unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    df_cen = pd.DataFrame([{"Cenário": c.nome, "Período": c.periodo, "Descrição": c.descricao}
    ...
    if st.button("💥  Rodar todos os stress tests", type="primary", use_container_width=True):
        ...
        with col_g1:
            ...
            fig_bar.update_layout(barmode="group",
                                  xaxis=dict(gridcolor=BORDER, tickangle=-20, tickfont=dict(size=10)),
        ...
        with col_g2:
            fig_r = go.Figure(go.Bar(
                x=df_res["Impacto (R$)"].tolist(), y=df_res["Cenário"].tolist(),
                orientation="h",
            ...
            fig_r.update_layout(
                title=dict(text="Impacto direto (R$)", font=dict(color=TEXT_SEC, size=12)),
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with col1:
        st.markdown(f'<div style="color:{MUTED};font-size:0.72rem;text-transform:uppercase;letter-spacing:0.08em;font-family:DM Mono,monospace;margin-bottom:0.5rem">Alocação por classe</div>', unsafe_allow_html=True)
    ...
    with col2:
        st.markdown(f'<div style="color:{MUTED};font-size:0.72rem;text-transform:uppercase;letter-spacing:0.08em;font-family:DM Mono,monospace;margin-bottom:0.5rem">P&L por ativo</div>', unsafe_allow_html=True)
        df_pl = df.sort_values("P&L (R$)")
        fig_pl = go.Figure(go.Bar(
            x=df_pl["P&L (R$)"], y=df_pl["Ticker"], orientation="h",
            marker_color=[NEG if v < 0 else POS for v in df_pl["P&L (R$)"]],
            opacity=0.85,
    ...
    # ── Volatilidade ──────────────────────────────────────────────────────────
    st.divider()
    st.markdown(f'<div style="color:{MUTED};font-size:0.72rem;text-transform:uppercase;letter-spacing:0.08em;font-family:DM Mono,monospace;margin-bottom:0.5rem">Volatilidade anualizada por ativo</div>', unsafe_allow_html=True)
    with st.spinner(""):
        ...
        for ativo in carteira.ativos:
            if is_tesouro(ativo.ticker):
                ...
                retornos = get_historical_returns(ativo.ticker)
                vol = float(np.std(retornos) * np.sqrt(252) * 100) if retornos is not None and len(retornos) > 10 else None
            if vol is not None:
                vols.append({"Ticker": ativo.ticker, "Volatilidade (%)": vol})
    ...
    if vols:
        ...
        fig_vol.update_layout(**plotly_layout(), xaxis_ticksuffix="%",
                              margin=dict(t=30, b=20, l=10, r=80))
        st.plotly_chart(fig_vol, use_container_width=True, key="vol_analise")
        st.markdown(
            f'<span style="color:#4ade80;font-family:DM Mono,monospace;font-size:0.75rem">■ Baixo &lt;15%</span>&nbsp;&nbsp;'
            f'<span style="color:#fbbf24;font-family:DM Mono,monospace;font-size:0.75rem">■ Moderado 15–30%</span>&nbsp;&nbsp;'
            f'<span style="color:#fb923c;font-family:DM Mono,monospace;font-size:0.75rem">■ Alto 30–50%</span>&nbsp;&nbsp;'
            f'<span style="color:#f87171;font-family:DM Mono,monospace;font-size:0.75rem">■ Muito alto &gt;50%</span>',
            unsafe_allow_html=True)

    # ── Benchmarks ────────────────────────────────────────────────────────────
    st.divider()
    st.markdown(f'<div style="color:{MUTED};font-size:0.72rem;text-transform:uppercase;letter-spacing:0.08em;font-family:DM Mono,monospace;margin-bottom:0.5rem">Retorno acumulado vs benchmarks — 12 meses</div>', unsafe_allow_html=True)
    with st.spinner(""):
    ...
    if acum_cart is not None:
        ...
        fig_acum.update_layout(**PLOT_LAYOUT, yaxis_ticksuffix="%",
                               legend=dict(bgcolor=f"rgba(30,41,59,0.95)", bordercolor=BORDER,
    ...
    # ── Correlação ────────────────────────────────────────────────────────────
    st.divider()
    st.markdown(f'<div style="color:{MUTED};font-size:0.72rem;text-transform:uppercase;letter-spacing:0.08em;font-family:DM Mono,monospace;margin-bottom:0.5rem">Correlação entre ativos — 12 meses</div>', unsafe_allow_html=True)
    with st.spinner(""):
    ...
    if corr is not None and not corr.empty:
        ...
        st.caption("Verde = correlação positiva · Vermelho = correlação negativa")

    # ── Métricas de risco ────────────────────────────────────────────────────
    st.divider()
    st.markdown(f'<div style="color:{MUTED};font-size:0.72rem;text-transform:uppercase;letter-spacing:0.08em;font-family:DM Mono,monospace;margin-bottom:0.75rem">Métricas de risco — 12 meses</div>', unsafe_allow_html=True)
    with st.spinner(""):
    ...
    if metricas:
        ...
        r1.metric("Volatilidade", f"{metricas['vol_anual']*100:.1f}%",
                  help="Desvio padrão anualizado")
        r2.metric("Sharpe", f"{metricas['sharpe']:.2f}", delta=sharpe_label,
                  delta_color="normal" if metricas["sharpe"] >= 0.5 else "inverse",
                  help="Retorno ajustado ao risco vs CDI")
        r3.metric("Sortino", f"{metricas['sortino']:.2f}",
                  help="Sharpe penalizando só volatilidade negativa")
        r4.metric("Max Drawdown", f"{metricas['max_drawdown']*100:.1f}%", delta=dd_label,
                  delta_color="inverse" if abs(metricas["max_drawdown"]) > 0.10 else "normal")
        r5.metric("Beta vs Ibov",
                  f"{metricas['beta']:.2f}" if metricas["beta"] is not None else "—",
                  help="Beta > 1 = mais volátil que o Ibovespa")

        st.divider()
        st.markdown(f'<div style="color:{MUTED};font-size:0.72rem;text-transform:uppercase;letter-spacing:0.08em;font-family:DM Mono,monospace;margin-bottom:0.5rem">Drawdown histórico</div>', unsafe_allow_html=True)
        fig_dd = go.Figure()
        fig_dd.add_trace(go.Scatter(
            x=metricas["serie_drawdown"].index, y=metricas["serie_drawdown"].values * 100,
            mode="lines", fill="tozeroy", fillcolor="rgba(248,113,113,0.1)",
        ...
        with col_int1:
            st.markdown(f'<div class="info-card"><p style="color:{MUTED};font-size:0.68rem;text-transform:uppercase;letter-spacing:0.1em;margin:0 0 8px;font-family:DM Mono,monospace">Sharpe · {sharpe_label}</p><p style="color:{TEXT_SEC};font-size:0.85rem;margin:0">Para cada unidade de risco, a carteira gerou <strong style="color:{TEXT_PRI}">{metricas["sharpe"]:.2f}x</strong> acima do CDI.{"" if metricas["sharpe"] >= 1 else " Considere reduzir ativos de alta volatilidade."}</p></div>', unsafe_allow_html=True)
        with col_int2:
            st.markdown(f'<div class="info-card"><p style="color:{MUTED};font-size:0.68rem;text-transform:uppercase;letter-spacing:0.1em;margin:0 0 8px;font-family:DM Mono,monospace">Max Drawdown · {dd_label}</p><p style="color:{TEXT_SEC};font-size:0.85rem;margin:0">Maior queda registrada: <strong style="color:{TEXT_PRI}">{metricas["max_drawdown"]*100:.1f}%</strong> em {metricas["max_dd_date"].strftime("%d/%m/%Y")}.</p></div>', unsafe_allow_html=True)

    # ── Concentração ──────────────────────────────────────────────────────────
    st.divider()
    col3, col4 = st.columns(2)
    with col3:
        st.markdown(f'<div style="color:{MUTED};font-size:0.72rem;text-transform:uppercase;letter-spacing:0.08em;font-family:DM Mono,monospace;margin-bottom:0.5rem">Peso por ativo</div>', unsafe_allow_html=True)
        ...
        fig_peso.update_layout(**PLOT_LAYOUT, xaxis_ticksuffix="%",
                               margin=dict(t=20, b=20, l=10, r=60))
        st.plotly_chart(fig_peso, use_container_width=True, key="peso_analise")

    with col4:
        st.markdown(f'<div style="color:{MUTED};font-size:0.72rem;text-transform:uppercase;letter-spacing:0.08em;font-family:DM Mono,monospace;margin-bottom:0.5rem">Concentração</div>', unsafe_allow_html=True)
        ...
        st.markdown(f'<div style="color:{MUTED};font-size:0.72rem;text-transform:uppercase;letter-spacing:0.08em;font-family:DM Mono,monospace;margin:1rem 0 0.5rem">Por classe</div>', unsafe_allow_html=True)
        for classe, pct in sorted(carteira.alocacao_por_classe().items(), key=lambda x: -x[1]):
            st.markdown(
                ...
                f'<span style="color:{TEXT_SEC};font-size:0.82rem;font-family:DM Mono,monospace;font-weight:500">{pct:.1f}%</span>'
                f'</div>', unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with st.expander("📋 Contexto carregado", expanded=False):
        ...
        with col_ctx2:
            ...
            if n_sims:
                ...
                st.caption("Últimas: " + " · ".join(f'"{c[:20]}"' for c in ultimas))
            ...
            if carteira.ativos:
                with st.chat_message("assistant"):
                    resposta = st.write_stream(sugerir_rebalanceamento_stream(
                        carteira, st.session_state.historico_simulacoes,
                        st.session_state.benchmarks_cache,
                    ))
    ...
    if not st.session_state.historico_chat:
        st.markdown(f'<div style="color:{MUTED};font-size:0.72rem;text-transform:uppercase;letter-spacing:0.08em;font-family:DM Mono,monospace;margin-bottom:0.75rem">Sugestões</div>', unsafe_allow_html=True)
        SUGESTOES = [
            "Qual meu ativo mais arriscado?", "Como estou vs Ibovespa?",
            "Resuma os cenários simulados", "Onde estou concentrado demais?",
            "Me explica o VaR",
        ]
