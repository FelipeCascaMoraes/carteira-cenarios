...
[data-testid="stWidgetLabel"] p, [data-testid="stWidgetLabel"] label {{ color: {TEXT_SEC} !important; }}
...
[data-testid="metric-container"] {{ background: {SURFACE}; border: 1px solid {BORDER}; border-radius: 12px; padding: 1rem 1.2rem !important; transition: border-color 0.2s; }}
...
.stButton > button:not([kind="primary"]) {{ background: {SURFACE} !important; color: {TEXT_SEC} !important; border: 1px solid {BORDER} !important; border-radius: 8px !important; transition: all 0.2s; }}
...
[data-testid="stTextInput"] input, [data-testid="stNumberInput"] input {{ background: {SURFACE} !important; border: 1px solid {BORDER} !important; color: {TEXT_PRI} !important; border-radius: 8px !important; font-family: 'DM Mono', monospace !important; }}
::placeholder {{ color: {MUTED} !important; opacity: 1; }}
.stSelectbox > div > div {{ background: {SURFACE} !important; border: 1px solid {BORDER} !important; border-radius: 8px !important; color: {TEXT_PRI} !important; }}
[data-testid="stSelectboxVirtualDropdown"] li, [data-testid="stSelectboxVirtualDropdown"] span {{ color: {TEXT_PRI} !important; background: {SURFACE} !important; }}
...
[data-testid="stChatMessage"] {{ background: {SURFACE} !important; border: 1px solid {BORDER} !important; border-radius: 12px !important; }}
[data-testid="stChatMessage"] p, [data-testid="stChatMessage"] span, [data-testid="stChatMessage"] div {{ color: #cbd5e1 !important; }}
textarea {{ background: {SURFACE} !important; border: 1px solid {BORDER} !important; color: {TEXT_PRI} !important; border-radius: 8px !important; font-family: 'Syne', sans-serif !important; }}
[data-testid="stProgress"] > div > div {{ background: #3b82f6 !important; }}

.var-card {{ background: #450a0a; border: 1px solid #7f1d1d; border-radius: 10px; padding: 1rem 1.2rem; }}
.var-card p {{ color: #fca5a5 !important; }}
.var-card h3 {{ color: #f87171 !important; }}
.badge-positivo {{ display: inline-block; background: #052e16; color: #4ade80; border: 1px solid #166534; border-radius: 20px; padding: 3px 14px; font-family: 'DM Mono', monospace; font-size: 0.85rem; font-weight: 600; }}
.badge-negativo {{ display: inline-block; background: #450a0a; color: #f87171; border: 1px solid #7f1d1d; border-radius: 20px; padding: 3px 14px; font-family: 'DM Mono', monospace; font-size: 0.85rem; font-weight: 600; }}
.badge-neutro {{ display: inline-block; background: {SURFACE}; color: {TEXT_SEC}; border: 1px solid {BORDER}; border-radius: 20px; padding: 3px 14px; font-family: 'DM Mono', monospace; font-size: 0.85rem; font-weight: 600; }}
</style>
...
if pagina == "🏠  Carteira":
    ...
    with st.expander("➕  Adicionar ativo", expanded=not carteira.ativos):
        ...
        with c1:
            if tipo == "Tesouro Direto":
                ...
                classe = st.selectbox("Classe", [k for k in CLASSES if k != "tesouro"], format_func=lambda x: CLASSES[x])
        with c2:
            if tipo == "Tesouro Direto":
    ...
    m3.metric("P&L (R$)",       f"R$ {carteira.pl_total_reais:+,.2f}", delta=f"{carteira.pl_total_pct:+.2f}%")
    ...
    if acum is not None and len(acum) > 1:
        ...
        fig_evol.add_hline(y=valor_inicial, line_color=MUTED, line_dash="dash", line_width=1,
                           annotation_text="Investido", annotation_font_color=MUTED,
                           annotation_position="bottom right")
        fig_evol.update_layout(**plotly_layout())
        fig_evol.update_layout(yaxis_title="Valor (R$)", yaxis_tickprefix="R$ ",
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if st.button("🚀  Simular cenário", type="primary", use_container_width=True):
        ...
        st.markdown(
            ...
            f'<p style="font-size:0.72rem;letter-spacing:0.08em;text-transform:uppercase;margin:0 0 4px">⚠️ Valor em Risco (VaR 90%)</p>'
            f'<h3 style="margin:0;font-family:DM Mono,monospace">R$ {var_reais:,.2f} &nbsp;<span style="font-size:1rem">({var_pct:.1f}%)</span></h3>'
        ...
        with col_hist:
            ...
            fig.add_trace(go.Histogram(
                ...
                marker=dict(color="#3b82f6", opacity=0.75, line=dict(color=BG, width=0.3)),
                name="Simulações",
            ))
    ...
    if st.button("💥  Rodar todos os stress tests", type="primary", use_container_width=True):
        ...
        with col_g1:
            ...
            for col, cor in [("P10 (%)", "#f87171"), ("P50 (%)", "#fbbf24"), ("P90 (%)", "#4ade80")]:
                fig_bar.add_trace(go.Bar(
                    name=col.replace(" (%)", ""), x=nomes, y=df_res[col].tolist(),
                    marker_color=cor, opacity=0.85,
        ...
        with col_g2:
            ...
            fig_r = go.Figure(go.Bar(
                ...
                orientation="h", marker_color=cores_r,
        ...
        with c_melhor:
            ...
            st.success(f"**🟢 Cenário menos severo: {row['Cenário']}**\n\nMediano: **{row['P50 (%)']:+.1f}%** (R$ {row['P50 (R$)']:,.2f})\n\nOtimista: **{row['P90 (%)']:+.1f}%**")
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if vols:
        ...
        fig_vol = go.Figure(go.Bar(
            ...
            textposition="outside", textfont=dict(size=10, color=MUTED),
            hovertemplate="<b>%{y}</b><br>Volatilidade: %{x:.1f}%<extra></extra>",
        ))
        ...
        fig_vol.update_layout(xaxis_title="Volatilidade anual (%)", xaxis_ticksuffix="%",
                              margin=dict(t=30, b=20, l=10, r=80))
        st.plotly_chart(fig_vol, use_container_width=True, key="vol_analise")
        st.markdown(
            f'<span style="color:#4ade80">■</span> Baixo (&lt;15%)&nbsp;&nbsp;'
    ...
    if acum_cart is not None:
        ...
        fig_acum.update_layout(
            yaxis_title="Retorno acumulado (%)", yaxis_ticksuffix="%",
            legend=dict(bgcolor="rgba(30,41,59,0.9)", bordercolor=BORDER,
                        borderwidth=1, font=dict(size=12, color=TEXT_PRI)),
    ...
    if metricas:
        ...
        r3.metric("Sortino Ratio", f"{metricas['sortino']:.2f}",
        ...
        fig_dd.add_trace(go.Scatter(
            ...
            mode="lines", fill="tozeroy", fillcolor="rgba(248, 113, 113, 0.15)",
        ...
        with col_int1:
            st.markdown(
                f'<div style="background:{SURFACE};border:1px solid {BORDER};border-radius:10px;padding:1rem">'
                f'<p style="color:{MUTED};font-size:0.72rem;text-transform:uppercase;letter-spacing:0.08em;margin:0 0 8px">Sharpe · {sharpe_label}</p>'
                f'<p style="color:{TEXT_SEC};font-size:0.85rem;margin:0">'
                f'Para cada unidade de risco assumida, a carteira gerou <strong>{metricas["sharpe"]:.2f}x</strong> de retorno acima do CDI.'
                f'{"" if metricas["sharpe"] >= 1 else " Considere reduzir ativos de alta volatilidade."}'
                f'</p></div>', unsafe_allow_html=True)
        with col_int2:
            st.markdown(
                f'<div style="background:{SURFACE};border:1px solid {BORDER};border-radius:10px;padding:1rem">'
                f'<p style="color:{MUTED};font-size:0.72rem;text-transform:uppercase;letter-spacing:0.08em;margin:0 0 8px">Max Drawdown · {dd_label}</p>'
                f'<p style="color:{TEXT_SEC};font-size:0.85rem;margin:0">'
                f'A maior queda da carteira foi de <strong>{metricas["max_drawdown"]*100:.1f}%</strong>, '
    ...
    with col3:
        ...
        fig_peso.update_layout(xaxis=dict(gridcolor=BORDER, zeroline=False, ticksuffix="%"),
    ...
    with col4:
        ...
        for classe, pct in sorted(carteira.alocacao_por_classe().items(), key=lambda x: -x[1]):
            st.markdown(
                f'<div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid {BORDER}">'
